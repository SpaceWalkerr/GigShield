...
export async function syncOnboardingToBackend(formData) {
  ...
  if (profileError) {
    return { ok: false, backend: false, error: profileError.message };
  }
  ...
  if (workerProfileError) {
    return { ok: false, backend: false, error: workerProfileError.message };
  }
  ...
  if (platforms.length > 0) {
    ...
    if (platformError) {
      return { ok: false, backend: false, error: platformError.message };
    }
  }
  ...
  if (policyError) {
    return { ok: false, backend: false, error: policyError.message };
  }
  ...
  if (pricingError) {
    return { ok: false, backend: false, error: pricingError.message };
  }
