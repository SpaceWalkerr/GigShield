...
function buildFallbackWorkerId(authUserId) {
  return `GS-${String(authUserId || "worker")
    .slice(0, 8)
    .toUpperCase()}`;
}

function buildSessionPayload({ authSession, profile, workerProfile, policy }) {
  ...
  const selectedPlanId =
    policy?.plan_id || getSession()?.selectedPlanId || planDetails[0].id;
  const selectedPlan =
...
export async function hydrateSessionFromSupabase({
  ...
  try {
    ...
    const authSession =
      providedAuthSession ||
      (await Promise.race([supabase.auth.getSession(), timeoutPromise])).data
        ?.session;
    ...
    const fetchAll = Promise.all([
      ...
      safeQuery(
        supabase
          ...
          .select(
            "worker_id, city, work_pattern, weekly_earnings_band, preferred_zones",
          )
    ...
    const results = await Promise.race([
      ...
      new Promise((res) =>
        setTimeout(
          () => res([{ data: null }, { data: null }, { data: null }]),
          3500,
        ),
      ),
    ]);

    const [{ data: profile }, { data: workerProfile }, { data: policy }] =
      results;
...
export async function signInWithEmail({ email, password }) {
  ...
  // Use the auth session returned by Supabase so login works even if
  // profile hydration is slow or temporarily unavailable.
  const authSession =
    data?.session || (await supabase.auth.getSession()).data?.session;
  const hydrated = await hydrateSessionFromSupabase({ authSession });
