...
function SignInPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fb] flex font-sans selection:bg-gray-900 selection:text-white">
      ...
      <div className="hidden lg:flex lg:w-3/5 flex-col justify-between p-16 relative overflow-hidden mesh-gradient">
          ...
          <div className="mt-24 space-y-8">
            <div className="space-y-3">
              <p className="text-[11px] font-black uppercase tracking-[0.4em] text-gray-500 animate-enter">
                {selectLabel(languageMode, "Parametric Income Protection", "पैरामेट्रिक आय सुरक्षा")}
              </p>
              <h2 className="text-7xl font-black text-gray-900 leading-[0.95] tracking-tightest font-archivo">
                ...
                <span className="text-blue-600 italic tracking-tighter">{selectLabel(languageMode, "rider.", "राइडर।")}</span>
              </h2>
            </div>
            
            <p className="text-gray-500 text-xl leading-relaxed max-w-md font-medium">
              {selectLabel(languageMode, "Secure your earnings against weather disruptions and platform outages automatically.", "मौसम और प्लेटफॉर्म बाधाओं के खिलाफ अपनी कमाई को स्वचालित रूप से सुरक्षित करें।")}
            </p>
          </div>
        </div>
        ...
        <div className="relative z-10 flex flex-col gap-8">
          <div className="glass-card rounded-[32px] p-8 w-fit flex items-center gap-6 group hover:translate-y-[-4px] transition-all duration-500">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-1">{selectLabel(languageMode, "Active Coverage", "सक्रिय सुरक्षा")}</p>
              <p className="text-lg font-bold text-gray-900 uppercase tracking-tight">{selectedPlan.name} {selectLabel(languageMode, "Plan", "योजना")}</p>
              <p className="text-sm font-semibold text-blue-600">{formatCurrency(calculateWeeklyPremium({ basePremium: selectedPlan.weeklyPremium, platformCount: 2, riskLevel: "Medium" }).adjustedPremium)} / {selectLabel(languageMode, "week", "सप्ताह")}</p>
            </div>
          </div>
      ...
      <div className="w-full lg:w-2/5 flex flex-col justify-center items-center px-6 py-12 sm:px-12 relative">
        ...
        <div className="w-full max-w-md space-y-12">
          <div className="text-center space-y-4">
            <h1 className="text-4xl font-black text-gray-900 tracking-tight font-archivo">{selectLabel(languageMode, "Sign In", "साइन इन करें")}</h1>
            <p className="text-sm text-gray-500 font-medium">
              {selectLabel(languageMode, "New to GigShield?", "GigShield पर नए हैं?")} {" "}
              <Link to={`/signup?plan=${selectedPlanId}`} className="text-gray-900 font-bold hover:underline underline-offset-4 decoration-2">
          ...
          <div className="glass-card rounded-[40px] p-10 space-y-6">
            ...
            {!supabase && (
              <div className="p-4 rounded-2xl border border-orange-100 bg-orange-50 text-xs font-bold text-orange-600 animate-enter">
                Supabase is not configured. Please check your .env file or use Demo Mode.
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const { error } = await supabase.auth.signInWithOAuth({ provider: "google", options: { redirectTo: `${window.location.origin}/signin?plan=${selectedPlanId}`, queryParams: { prompt: "select_account" } } });
              ...
              >
                ...
                <span>{selectLabel(languageMode, "Explore as Demo User", "डेमो यूजर के रूप में देखें")}</span>
              </button>
            </div>
          </div>
